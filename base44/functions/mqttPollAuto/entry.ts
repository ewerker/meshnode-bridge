        ...
        if (matches.length > 0) {
          if (newHasOriginalSender) {
            ...
            if (losers.length === matches.length) {
              ...
              console.log('[MQTT-AUTO] content-dedup replace: kept new (with original sender), dropped', losers.length, 'older copies');
            } else {
              console.log('[MQTT-AUTO] content-dedup skip: another copy with original sender already exists');
              continue;
            }
          } else {
