...
const BUCKETS = [
  ...
  { label: '1–6h', maxSec: 21600, color: 'hsl(30, 80%, 55%)' },
  { label: '6–24h', maxSec: 86400, color: 'hsl(45, 90%, 50%)' },
  { label: '1–7d', maxSec: 604800, color: 'hsl(186, 100%, 42%)' },
  { label: '>7d', maxSec: Infinity, color: 'hsl(160, 60%, 45%)' },
];
...
export default function UptimeChart({ nodes }) {
  ...
  const data = BUCKETS.map(b => {
    const count = withUptime.filter(n => n.uptime_seconds >= prevMax && n.uptime_seconds < b.maxSec).length;
    const item = { name: b.label, count, color: b.color };
    prevMax = b.maxSec;
    return item;
  });

  const noData = nodes.length - withUptime.length;
  if (noData > 0) {
    data.push({ name: 'No data', count: noData, color: 'hsl(222, 30%, 25%)' });
  }
